...
printf "%-34s %9s %9s %9s\n" "screenshot" "avif" "webp" "png"
total_master=0; total_avif=0; total_webp=0; total_png=0

for master in "$SRC"/*@2x.png; do
  base="$(basename "$master" "@2x.png")"

  sips -s format avif -s formatOptions 60 "$master" --out "$OUT/$base@2x.avif" >/dev/null
  sips -s format avif -s formatOptions 60 --resampleWidth 1280 "$master" --out "$OUT/$base.avif" >/dev/null

  cwebp -q 80 -quiet "$master" -o "$OUT/$base@2x.webp"
  cwebp -q 80 -quiet -resize 1280 0 "$master" -o "$OUT/$base.webp"

  # The fallback only reaches a browser that reads neither format above, so it
  # is quantised: flat window chrome takes 256 colours without visible banding.
  magick "$master" -colors 256 "$OUT/$base@2x.png"

  m=$(stat -f%z "$master"); a=$(stat -f%z "$OUT/$base@2x.avif")
  w=$(stat -f%z "$OUT/$base@2x.webp"); p=$(stat -f%z "$OUT/$base@2x.png")
  total_master=$((total_master+m)); total_avif=$((total_avif+a))
  total_webp=$((total_webp+w)); total_png=$((total_png+p))
  printf "%-34s %8dK %8dK %8dK\n" "$base" $((a/1024)) $((w/1024)) $((p/1024))
done
