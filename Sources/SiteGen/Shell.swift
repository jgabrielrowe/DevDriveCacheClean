...
public struct Shell: Sendable {
    ...
    /// The analytics label for a shell link, or "" for the ones that carry none.
    ///
    /// Declared once so the masthead and the footer cannot disagree. Both point
    /// at the download page from every page on the site, and neither used to be
    /// counted, while thirteen in-page buttons were — so `download-cta` read as
    /// a complete tally of "headed for the download page" while missing the two
    /// links present on all 32 pages.
    private func navEvent(for path: String) -> String {
        path == "download/" ? " data-umami-event=\"download-cta\"" : ""
    }
    ...
    private func navHTML(current: String, root: String) -> String {
        Site.nav.enumerated().map { index, item in
            ...
            // count.
            return "      <a href=\"\(root)\(item.path)\" style=\"--i:\(index)\"\(mark)\(navEvent(for: item.path))>\(item.title)</a>\n"
        }.joined()
    }
    ...
    private func footerColumnHTML(heading: String, root: String) -> String {
        ...
        let items = group.links
            .map { "<li><a href=\"\(root)\($0.path)\"\(navEvent(for: $0.path))>\($0.title)</a></li>" }
            .joined()
