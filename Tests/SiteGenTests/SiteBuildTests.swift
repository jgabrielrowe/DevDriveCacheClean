...
/// A WebP sibling on disk that no page references is weight nobody is served.
@Test func everyGeneratedWebPIsReferencedByAPage() throws {
    let dir = repoRoot().appending(path: "Site/assets/screenshots")
    let webp = try FileManager.default.contentsOfDirectory(atPath: dir.path())
...
@Test func everyScreenshotMasterMatchesItsGeneratedDerivatives() throws {
    ...
    for file in onDisk {
        ...
        // Every master owes three served files: WebP at both densities, plus the
        // PNG the `<img>` falls back to.
        let base = String(file.dropLast("@2x.png".count))
        for name in ["\(base).webp", "\(base)@2x.webp", "\(base)@2x.png"] {
...
@Test func everyServedScreenshotIsReferencedByAPage() throws {
    ...
    // Eleven masters, three served files each: WebP at both densities and the
    // PNG the `<img>` falls back to.
